...
export function useEyeRBluetooth() {
  ...
  const discoverNotifyingCharacteristic = useCallback(
    async (
      server: BluetoothRemoteGATTServer,
      attemptToken: number,
    ): Promise<BluetoothRemoteGATTCharacteristic> => {
      console.log(
        "Starting cross-service auto-discovery for notifying characteristics...",
      );
      ...
      try {
        ...
        for (const service of services) {
          ...
          try {
            ...
            console.log(
              `Service ${service.uuid}: found ${characteristics.length} characteristics`,
            );

            for (const char of characteristics) {
              if (char.properties.notify) {
                console.log(
                  `Found notifying characteristic: ${char.uuid} in service ${service.uuid}`,
                );
                return char;
              }
            }
            ...
            console.warn(
              `Failed to scan characteristics for service ${service.uuid}:`,
              err,
            );
          }
        }
        ...
        throw new Error(
          "No notify-capable characteristic found on this device.",
        );
      } catch (err: any) {
        ...
        throw new Error(
          `Failed to discover services and characteristics: ${err.message || "Unknown error"}`,
        );
      }
    },
    [],
  ...
  const enableNotifications = useCallback(
    async (
      ...
      try {
        ...
        throw new Error(
          `Failed to start notifications: ${err.message || "Unknown error"}`,
        );
      }
      ...
      try {
        ...
        console.warn(
          "CCCD descriptor write failed (this may be normal):",
          cccdErr.message || cccdErr,
        );
      }
  ...
  const connect = useCallback(
    async (_options: ConnectOptions = {}) => {
      ...
      try {
        ...
        console.log(
          "Requesting BLE device (acceptAllDevices, optionalServices: NUS)...",
        );
        ...
        try {
          ...
          if (nusService) {
            ...
            const txCharacteristic = await nusService.getCharacteristic(
              NUS_TX_CHARACTERISTIC_UUID,
            );
            ...
            console.warn(
              "NUS service not found via universal discovery, falling back to cross-service discovery...",
            );
            characteristic = await discoverNotifyingCharacteristic(
              server,
              currentAttemptToken,
            );
            console.log("Step 3: Service Found");
    ...
    [
      isSupported,
      cleanupConnection,
      discoverNotifyingCharacteristic,
      enableNotifications,
    ],
  );
