...
export function generateLocalSleepSchedule(
  ...
  const totalBPM = dataToAnalyze.reduce(
    (sum, point) => sum + point.blinkRate,
    0,
  );
  ...
  if (averageBPM < 10) {
    recommendations.push(
      "Extremely low blink rate detected - immediate rest recommended",
    );
  ...
  if (variability > 20) {
    recommendations.push(
      "High variability in blink rate - consider stress management techniques",
    );
  }
  ...
  if (currentHour >= 22 || currentHour < 6) {
    recommendations.push(
      "Late night/early morning detected - prioritize sleep soon",
    );
  } else if (currentHour >= 14 && currentHour < 16) {
    recommendations.push(
      "Afternoon dip period - short break or light exercise recommended",
    );
  }
