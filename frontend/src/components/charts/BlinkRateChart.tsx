...
export default function BlinkRateChart({ data, title = 'Blink Rate History', lightLevelData }: BlinkRateChartProps) {
  ...
  useEffect(() => {
    if (!lightCanvasRef.current || !lightLevelData || lightLevelData.length === 0) return;
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Activity className="h-5 w-5" />
          {title} ({hasBlinkData ? data.length : (lightLevelData?.length ?? 0)} readings)
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {hasLightData && (
          <>
            <p className="text-xs font-medium text-muted-foreground">Real-time Light Level (VAL)</p>
            <canvas
              ref={lightCanvasRef}
              width={800}
              height={200}
            ...
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              ...
              <span>Latest: {formatTime(lightLevelData![lightLevelData!.length - 1].timestamp)}</span>
            </div>
          </>
        )}
      </CardContent>
